...
import { MapPin, TrendingUp, Building2, Sparkles, ArrowUpRight, ExternalLink } from "lucide-react";
...
// Zillow search URLs for each property
const zillowUrls: Record<string, string> = {
  "5901 Filaree Heights, Malibu": "https://www.zillow.com/homes/5901-Filaree-Heights-Malibu-CA_rb/",
  "20737 Cool Oak Way, Malibu": "https://www.zillow.com/homes/20737-Cool-Oak-Way-Malibu-CA_rb/",
  "2460 Sunset Plaza Dr, Los Angeles": "https://www.zillow.com/homes/2460-Sunset-Plaza-Dr-Los-Angeles-CA_rb/",
  "20647 Seaboard Rd, Malibu": "https://www.zillow.com/homes/20647-Seaboard-Rd-Malibu-CA_rb/",
  "8818 Rising Glen Place, Los Angeles": "https://www.zillow.com/homes/8818-Rising-Glen-Pl-Los-Angeles-CA_rb/",
  "1394 Casiano Rd, Los Angeles": "https://www.zillow.com/homes/1394-Casiano-Rd-Los-Angeles-CA_rb/",
  "1061 Loma Vista Dr, Beverly Hills": "https://www.zillow.com/homes/1061-Loma-Vista-Dr-Beverly-Hills-CA_rb/",
  "4965 Calvin Avenue, Tarzana": "https://www.zillow.com/homes/4965-Calvin-Ave-Tarzana-CA_rb/",
};
...
export const DealCaseStudies = ({ deal }: DealCaseStudiesProps) => {
  ...
  const getZillowUrl = (address: string): string => {
    return zillowUrls[address] || `https://www.zillow.com/homes/${encodeURIComponent(address)}_rb/`;
  };
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="py-24 relative">
        {/* Dark Navy Gradient Background with violet accent */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#0a0f1a] via-[#0d1424] to-[#111827]" />
        
        {/* Atmospheric Effects with violet */}
        <div className="absolute inset-0">
          {/* Top violet glow - enhanced */}
          <div className="absolute -top-32 -right-32 w-[600px] h-[400px] bg-gradient-to-bl from-violet-800/30 via-violet-700/15 to-transparent rounded-full blur-3xl" />
          {/* Left violet accent */}
          <div className="absolute top-1/2 -left-20 w-[400px] h-[400px] bg-gradient-to-r from-violet-900/20 via-indigo-900/15 to-transparent rounded-full blur-3xl" />
          {/* Bottom gradient fade */}
          <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-[#0a0f1a]/80 to-transparent" />
          {/* Center violet glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] bg-violet-800/10 rounded-full blur-[100px]" />
        </div>
        
        {/* Top border line - violet accent */}
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          ...
          >
            <div>
              ...
              {/* Editorial Title with violet accent */}
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-light text-white leading-[1.1] mb-6">
                Case{" "}
                <span className="italic font-serif text-violet-400/90">
                  Studies
                </span>
              </h2>
            ...
            <div className="flex gap-4">
              <motion.div
                ...
                className="p-6 text-center bg-violet-900/30 border border-violet-700/40"
              >
                <p className="text-4xl font-light text-white mb-1">{deal.caseStudies.length}</p>
                <p className="text-[10px] text-violet-400/80 uppercase tracking-[0.2em]">Completed</p>
              </motion.div>
              ...
              <motion.div
                ...
                className="p-6 text-center bg-violet-800/20 border border-violet-600/30"
              >
                <p className="text-4xl font-light text-violet-300">${(totalProfit / 1000000).toFixed(1)}M</p>
                <p className="text-[10px] text-violet-400/70 uppercase tracking-[0.2em]">Total Profit</p>
              </motion.div>
            </div>
          </motion.div>
        </div>
                  ...
                  <div className="aspect-[4/3] rounded overflow-hidden shadow-2xl shadow-black/50 relative group border border-slate-700/50">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      >
                        ...
                        {/* Zillow Link Button */}
                        <motion.a
                          href={getZillowUrl(currentCase.address)}
                          target="_blank"
                          rel="noopener noreferrer"
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className="flex items-center gap-2 px-3 py-2 bg-violet-600/80 hover:bg-violet-500 backdrop-blur-sm rounded text-sm text-white border border-violet-500/50 transition-all duration-300"
                        >
                      ...
                      >
                        <div className="relative z-10">
                          <div className="flex items-center gap-2 mb-2">
                            {item.icon && <TrendingUp className="w-3.5 h-3.5 text-violet-400" />}
                            <p className="text-[10px] uppercase tracking-[0.15em] text-slate-500">
                              {item.label}
                            </p>
                          </div>
                          <p className={`text-xl font-light ${item.dark ? "text-violet-200" : "text-slate-200"}`}>
                            {item.value}
                          </p>
                        </div>
                      </motion.div>
