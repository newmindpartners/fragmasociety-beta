        ...
        <AnimatePresence mode="wait">
          {categories.length > 0 ? (
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                {categories.map((category, categoryIndex) => (
                  <motion.div
                    ...
                    className="group"
                  >
                    <div className="h-full bg-white rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-xl hover:border-slate-300/80 transition-all duration-500 overflow-hidden">
                      {/* Category Header */}
                      <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50/50 to-white">
                        <div className="flex items-center gap-4">
                          <motion.div
                            whileHover={{ scale: 1.05, rotate: 3 }}
                            className="w-12 h-12 rounded-xl bg-gradient-to-br from-turquoise/15 to-cyan-400/10 border border-turquoise/20 flex items-center justify-center"
                          >
                            <category.icon className="w-5 h-5 text-turquoise" />
                          </motion.div>
                          <div>
                        ...
                        >
                          {category.faqs.map((faq, faqIndex) => (
                            <AccordionItem
                              ...
                              className="border border-slate-100 rounded-xl px-4 bg-white hover:bg-slate-50/50 data-[state=open]:bg-slate-50 data-[state=open]:border-turquoise/20 transition-all duration-300"
                            >
                              <AccordionTrigger className="text-left text-sm font-medium text-slate-700 hover:no-underline hover:text-turquoise transition-colors py-4 [&[data-state=open]]:text-turquoise">
                                <HighlightText text={faq.question} highlight={searchQuery} />
                              </AccordionTrigger>
                              <AccordionContent className="text-sm text-slate-600 leading-relaxed pb-4 pr-4">
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-slate-900 mb-3">No results found</h3>
